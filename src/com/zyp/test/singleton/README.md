 ...
 *
 * 优点：线程安全；延迟加载；效率较高。
 * 但是这一种在java里面是实现不了，jvm存在指令重排的问题。
 */
class Singleton6 {
...
这里顺便说一下第六种双重检查，因为jvm的指令重排会导致这里也有问题，jvm指令重排是指在执行结果不会改变的情况下，JVM为了提高程序的执行效率会对指令进行重排序。，实际上在jvm中，`singleton = new Singleton6();`会分成下面三句话：
> ①分配空间给对象  
  ②在空间内创建对象  
  ③将对象赋值给引用singleton

可以看到2是依赖1的，所以2一定是在1之后执行，但是3和2不存在依赖性，也就是说执行顺序可能是1->3->2，如果是单线程执行那么如果后续程序使用到了singleton，JVM会保证你使用singleton的时候是初始化完成的，但是现在在synchronized块之外有其它线程“虎视眈眈”，获取到锁的线程如果按照1->3->2的顺序执行，那在执行3的时候会store-write，即将值写回主内存，则其它线程会读到最新的singleton值，而现在这个singleton指向的是一个不完全的对象，即不安全对象，也不可用，使用这个对象是有危险的，此时构造对象的线程还没有释放锁，其它线程进行第一次检查的时候，`null == sole`的结果是false，会返回这个对象，造成程序的异常。 

当然我们可以使用volatile来避免这个问题，有兴趣的可以自行查阅。
